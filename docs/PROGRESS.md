...
## 2026-09-23：JSON 单工作区与收起侧栏

- 用户要求最少界面占用、单框切换文本/树形。已移除JSON重复标题、简介和双栏卡片；紧凑工具栏下面的单个编辑区占满剩余视口，内容在内部滚动。保留可访问H1及原SEO元数据。
- 文本视图直接编辑当前文档；树形按需展开；格式化进入树形首层，压缩进入文本，校验不改内容。复制包含当前完整文档，错误保留输入，支持清空、一次格式化/压缩撤销及Ctrl/Cmd+Enter。快捷键切换后焦点进入树，返回文本自动聚焦编辑器。
- 桌面工具栏默认56px图标窄栏，悬停/键盘聚焦展开为268px覆盖层，不挤动内容；手机保留菜单抽屉。英文/中文同步。
- 54项Node测试、34页正式构建通过；独立代码审查无高/中优先级问题。浏览器覆盖1280×900、900×700、390×844、844×390；2万条约1.7MB JSON按需渲染、完整复制、编辑/撤销/错误/XSS/键盘/侧栏与其他工具导航通过，未见JS错误或页面横向溢出。报告docs/json-workspace-browser-checks.json。手机为Chromium视口模拟，并非实机性能测试。
- 功能提交60bffbe已推送main并确认正式站生效：34页HTTP200、canonical/新版资源引用/可索引状态正确，7个脚本及样式逐字节匹配正式构建，中英文JSON单工作区结构正确，sitemap/robots一致，未知路径404。证据docs/json-workspace-live-checks.json；未读取Cloudflare控制台部署编号，以正式资源指纹确认。此前GA4分支未混入本轮发布。
