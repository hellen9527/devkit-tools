...
function buildSite({outDir=path.join(root,'dist'),siteUrl=process.env.SITE_URL||''}={}){
 ...
 const jsonTreeUrl=asset('json-tree','js',read('assets/json-tree.js'));
 ...
 for(const language of ['en','zh']){
  ...
  const footer=`<footer class="site-footer"><a href="${route('home')}">${text('All tools','全部工具')}</a><a href="${route('about')}">${text('About','关于')}</a><a href="${route('privacy')}">${text('Privacy','隐私')}</a></footer>`;
  ...
  write(output('privacy'),page('privacy',text('Privacy — DevKit','隐私说明 — DevKit'),text('How DevKit handles tool inputs, language preference and website requests.','了解 DevKit 如何处理工具输入、语言偏好及网站访问请求。'),article(text('Privacy','隐私说明'),text('<h2>Tool inputs</h2><p>The tools process input locally in your browser. Application code does not upload tool inputs or store them in cookies or local storage. Navigating away or reloading clears the current tool state.</p><h2>Language preference</h2><p>Your manual language preference is saved in local storage as devkit-language. This stores only en or zh, never tool inputs. Clear website storage to remove it. If storage is disabled, language links still work. On the English homepage, your browser language provides the default when no preference is saved.</p><h2>Website delivery</h2><p>Cloudflare hosts the website. When you visit, your browser sends ordinary website requests to Cloudflare, including your IP address and browser request information. Hosting and security logs may contain this request information.</p><h2>Analytics and advertising</h2><p>This version does not include advertising scripts or application analytics. If that changes, this page will be updated.</p>','<h2>工具输入</h2><p>工具在浏览器本地处理输入。应用代码不会上传工具输入，也不会将其保存在 Cookie 或本地存储中。离开页面或刷新后，当前工具状态会清空。</p><h2>语言偏好</h2><p>手动选择的语言偏好保存在本地存储 devkit-language 中，仅记录 en 或 zh，不含工具输入。清除网站存储即可删除。禁用存储时仍可通过语言链接切换。在英文首页尚未保存偏好时，会按浏览器首选语言设置默认语言。</p><h2>网站访问</h2><p>本网站由 Cloudflare 托管。访问时，浏览器会向 Cloudflare 发送正常的网站请求，包括 IP 地址及浏览器请求信息。托管和安全日志可能包含这些请求信息。</p><h2>统计与广告</h2><p>当前版本未加入广告脚本或应用访问统计。未来如有变化，将更新本页说明。</p>'))));
